...
for(i in 1:6){
  funda_ds <- subset(funda_merge, annotations_super_coarse == clusters[i])
  Idents(funda_ds) <- funda_ds$MouseID
funda_bulk <- AggregateExpression(funda_ds, group.by = c("MouseID", "Diet"), 
                                return.seurat = T, assays = "RNA")

funda_bulk$Diet <- substr(colnames(funda_bulk),9,10)
Idents(funda_bulk) <- funda_bulk$Diet
Idents(funda_bulk)

  mrk_de <- FindAllMarkers(funda_bulk, 
                        min.pct = 0,
                        min.diff.pct = 0, 
                        min.cells.feature = 0,
                        min.cells.group = 0,
...
if(i == 1){
  ...
  mrk_all_bulk <- rbind(mrk_all_bulk, mrk_de)
}
}
