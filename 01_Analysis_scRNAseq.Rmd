...
To recapitulate the analysis with the processed data, please download the processed and annotated object available on Zenodo:
...
seu <- readRDS("data/exp11W3_init.rds")
...
```{r}
seu <- readRDS("data/exp11W4_init.rds")
...
### Markers of annotated populations
...
#write.csv(mrk_de_whole_data, "mrk_de_whole_data.csv")
getwd()
```
...
```{r}
funda_merge$annotations_coarse %>% table
```
```{r}
funda_merge@meta.data <- funda_merge@meta.data %>% 
  mutate(annotations_super_coarse = case_when(
    annotations_coarse %in% c( "CD4 Naive","CD4 Egr Myb", "CD4 Early act. 41BB CD69") ~ "CD4",
    annotations_coarse %in% c( "CD8 Naive","CD8 ISAGhi", "CD8 AIMT", "CD8 Egr") ~ "CD8",
    annotations_coarse %in% c( "CD4 Treg and Tfh") ~ "Treg",
    annotations_coarse %in% c( "Proliferating") ~ "Proliferating",
    annotations_coarse %in% c( "ILC", "Tgd and NKT ") ~ "Tgd NKT ILC",
    annotations_coarse %in% c( "NK cells") ~ "NK cells"
    
  ))

DimPlot(funda_merge, group.by = "annotations_super_coarse")
```


```{r}
clusters <- levels(factor(funda_merge$annotations_super_coarse))

for(i in 1:6){
  funda_ds <- subset(funda_merge, annotations_super_coarse == clusters[i])
  Idents(funda_merge) <- funda_merge$Diet
mrk_de <- FindAllMarkers(funda_ds, only.pos = TRUE, logfc.threshold = 0.1, min.pct = 0.05)
mrk_de$population <- clusters[i]
mrk_de$gene <- rownames(mrk_de)
if(i == 1){
  mrk_all <- mrk_de
} else {
  mrk_all <- rbind(mrk_all, mrk_de)
}
}
...
```{r}
mrk_all %>% dplyr::filter(population == "CD4")
mrk_all %>% dplyr::filter(population == "CD8")
mrk_all %>% dplyr::filter(population == "Treg")
mrk_all %>% dplyr::filter(population == "Proliferating")
mrk_all %>% dplyr::filter(population == "Tgd NKT ILC")
mrk_all %>% dplyr::filter(population == "NK cells")
...
mrk_all  %>% 
ggplot(aes(x = factor(gene), y = population)) + 
...
mrk_all %>% group_by(cluster, population) %>% tally %>% 
...
```
```{r}
mrk_all %>% group_by(cluster, gene) %>% tally %>% arrange(desc(n))
```
```{r}
...
for(i in 1:6){
  funda_ds <- subset(funda_merge, annotations_super_coarse == clusters[i])
  Idents(funda_merge) <- funda_merge$Diet
mrk_de <- FindAllMarkers(funda_ds, only.pos = TRUE, return.thresh = 1, min.diff.pct = 0, logfc.threshold = -Inf, min.pct = -Inf, )
mrk_de$population <- clusters[i]
mrk_de$gene <- rownames(mrk_de)
if(i == 1){
  mrk_fc_all <- mrk_de
} else {
  mrk_fc_all <- rbind(mrk_fc_all, mrk_de)
}
}


mrk_fc_all$cluster %>% table

mrk_fc_all %>% dplyr::filter(gene == "Lck")
```
...
signif_genes <- mrk_all %>% dplyr::filter(p_val_adj < 0.05) %>% pull(gene) %>% unique 

top50_genes <- mrk_all %>% group_by(population) %>% slice_max(order_by = avg_log2FC, n = 50) %>% pull(gene) %>% unique 
top50_genes

top30_genes <- mrk_all %>% group_by(population) %>% slice_max(order_by = avg_log2FC, n = 30) %>% pull(gene) %>% unique 

top20_genes <- mrk_all %>% group_by(population) %>% slice_max(order_by = avg_log2FC, n = 20) %>% pull(gene) %>% unique 

mtx_logfc <- mrk_fc_all %>% dplyr::filter(gene %in% signif_genes) %>% 
  mutate(avg_log2FC = ifelse(cluster == "Standard",avg_log2FC*-1, avg_log2FC)) %>% 
  dplyr::select(avg_log2FC, gene, population) %>% 
  pivot_wider(names_from = population, values_from = avg_log2FC) %>% 
  column_to_rownames("gene") %>% 
  as.matrix()


pheatmap::pheatmap(t(mtx_logfc), scale = "none", color=colorRampPalette(c("dodgerblue", "grey95", "indianred2",  "indianred2", "indianred2"))(50), 
         border_color = "white")
...
```{r}
mtx_logfc <- mrk_fc_all %>% dplyr::filter(gene %in% top30_genes) %>% 
  mutate(avg_log2FC = ifelse(cluster == "Standard",avg_log2FC*-1, avg_log2FC)) %>% 
  #mutate(avg_log2FC = ifelse(p_val_adj < 0.05,avg_log2FC, avg_log2FC/20)) %>% 
...
hm <- pheatmap::pheatmap(mtx_logfc, scale = "none", color=colorRampPalette(c("dodgerblue", "grey95", "indianred2", "indianred2", "indianred2"))(50), 
         border_color = "white")
```
...
mrk_fc_all %>% dplyr::filter(gene %in% top30_genes) %>% 
   mutate(avg_log2FC = ifelse(avg_log2FC > 1,1, avg_log2FC)) %>% 
  mutate(avg_log2FC = ifelse(cluster == "Standard",avg_log2FC*-1, avg_log2FC)) %>% 
  ggplot(aes(x = factor(population, levels = hm$tree_col$labels[hm$tree_col$order]), 
           y = factor(gene, levels = hm$tree_row$labels[hm$tree_row$order] ))) +
geom_point(aes(size = abs(avg_log2FC), colour = avg_log2FC)) + 
  theme_bw() +
scale_size_continuous(range=c(1,8)) +
  #geom_text(aes(label = p.adj.signif)) + 
  scale_colour_gradient2(low = "dodgerblue", mid = "white", high = "salmon", midpoint = 0) +
  theme_bw() + theme(axis.text.x = element_text(angle = 90))  +
xlab("") + ylab("") + ggtheme() + coord_flip()

ggsave(filename = "figs/genes_in_clusters.svg", height = 10, width = 60, 
       units = "cm")

```

```{r}
mrk_fc_all %>% dplyr::filter(gene %in% top20_genes) %>% 
   mutate(avg_log2FC = ifelse(avg_log2FC > 1,1, avg_log2FC)) %>% 
  mutate(avg_log2FC = ifelse(cluster == "Standard",avg_log2FC*-1, avg_log2FC)) %>% 
  ggplot(aes(x = factor(population, levels = hm$tree_col$labels[hm$tree_col$order]), 
           y = factor(gene, levels = hm$tree_row$labels[hm$tree_row$order] ))) +
...
ggsave(filename = "figs/genes_in_clusters.svg", height = 10, width = 40, 
       units = "cm")
...
```{r}
avgexp = AverageExpression(funda_merge, features = c("Ccl5","Gzma","Gzmb","Klre1","Cx3cr1"),
                           return.seurat = F, group.by = c("MouseID", "annotations_super_coarse"),
                          assay = "RNA")


df <- avgexp$RNA %>% as.data.frame() %>% rownames_to_column("gene") %>% pivot_longer(!gene, names_to = "MouseID_Population", values_to = "Expression") %>% 
  mutate(Mouse_ID = substr(MouseID_Population,1,7),
         Population = substr(MouseID_Population,9,50)) %>% mutate(
           Diet = ifelse(substr(MouseID_Population,7,7) %in% c(1:5),"GFD","STD")
         )

df

df %>% ggplot(aes(x = Diet, y = Expression)) +
  geom_point() +
  geom_boxplot() +
  facet_grid(rows = vars(gene), cols = vars(Population), scales = "free")
```
...
```{r}
m_df<- msigdbr::msigdbr(species = "Mus musculus", category = "C7")
...
mrk_fc_all %>% dplyr::filter(population == clusters[j]) %>% 
  mutate(avg_log2FC = ifelse(cluster == "Standard",avg_log2FC*-1, avg_log2FC)) %>% 
  arrange((avg_log2FC))


mrk_fc_all2 <- mrk_fc_all  %>% 
  mutate(avg_log2FC = ifelse(cluster == "Standard",avg_log2FC*-1, avg_log2FC)) %>% 
  arrange((avg_log2FC))
...
fc.df  <- mrk_fc_all2 %>% dplyr::filter(population == clusters[j])  %>%  arrange(desc(avg_log2FC)) %>% 
  dplyr::select(gene, avg_log2FC)
ranks<- deframe(fc.df)
      
fgseaRes <- fgsea(pathways = fgsea_sets, 
                 stats = ranks)
 fgseaRes$population <- clusters[j]
 return(fgseaRes)
    }
...
for(i in 1:length(clusters)){
  ...
  fc_final_df <- gsea_all_pathways(i)
  
  if(i == 1){
    fc_final_df_all <- fc_final_df
  } else {
    fc_final_df_all <- rbind(fc_final_df_all, fc_final_df)
  }
...
fc_final_df_all$population %>% table
...
fc_final_df_all %>% dplyr::filter(pathway %in% c("GSE28726_NAIVE_VS_ACTIVATED_CD4_TCELL_DN",
  "GSE14415_INDUCED_TREG_VS_TCONV_UP",
  "GOLDRATH_NAIVE_VS_EFF_CD8_TCELL_DN"
  )) %>% 
  mutate(signif = ifelse(padj < 0.05, "*","")) %>% 
  ggplot(aes(x = population, y = pathway)) +
  geom_point(aes(fill = NES, size = -log(padj)), shape = 21) +
  scale_fill_gradient2(low = "dodgerblue", mid = "white", high = "indianred2", midpoint = 0) + scale_size_continuous(range=c(4,7)) +
  geom_text(aes(label = signif)) + 
  scale_colour_gradient2(low = "dodgerblue", mid = "white", high = "salmon", midpoint = 0) +
  theme_bw() + ggtheme()  + theme(axis.text.x = element_text(angle = 45, hjust = 0.5, vjust = 0.5))

ggsave(filename = "figs/gsea_in_clusters_c7.svg", height = 8, width = 30, 
       units = "cm")
...
```{r}
m_df_h <- msigdbr::msigdbr(species = "Mus musculus", category = "H")
...
fgseaRes <- fgsea(pathways = fgsea_sets_h, 
                 stats = ranks)
 fgseaRes$population <- clusters[j]
 return(fgseaRes)
    }
...
for(i in 1:length(clusters)){
  ...
  fc_final_df_h <- gsea_all_pathways(i)
  
  if(i == 1){
    fc_final_df_all_h <- fc_final_df_h
  } else {
    fc_final_df_all_h <- rbind(fc_final_df_all_h, fc_final_df_h)
  }
...
fc_final_df_all_h$population %>% table
```


```{r fig.width = 14, fig.height = 5}
fc_final_df_all_h %>% arrange(padj)

df_filt <- fc_final_df_all_h %>% dplyr::filter(pathway %in% c("HALLMARK_MYC_TARGETS_V1",
...
df_filt$pathway <- factor(df_filt$pathway, levels = c("HALLMARK_MYC_TARGETS_V1",
  "HALLMARK_MTORC1_SIGNALING",
    "HALLMARK_IL2_STAT5_SIGNALING",
  "HALLMARK_INTERFERON_ALPHA_RESPONSE",
  "HALLMARK_INTERFERON_GAMMA_RESPONSE",
...
df_filt %>% 
  ggplot(aes(x = population, y = pathway)) +
  geom_point(aes(fill = NES, size = -log(padj)), shape = 21) +
  scale_fill_gradient2(low = "dodgerblue", mid = "white", high = "indianred2", midpoint = 0) + scale_size_continuous(range=c(4,12)) +
  geom_text(aes(label = signif)) + 
  scale_colour_gradient2(low = "dodgerblue", mid = "white", high = "salmon", midpoint = 0) +
  theme_bw() + ggtheme()  + theme(axis.text.x = element_text(angle = 45, hjust = 0.5, vjust = 0.5))
```
...
```{r}
ggsave(filename = "figs/gsea_in_clusters_h.svg", height = 12, width = 28, 
       units = "cm")
...
md <- md %>% mutate(annotations_fine = recode(seurat_clusters, 
                                                     ...
                                                     "4" = "3 Treg cytotoxic",
                                                     "5" = "5 Tfh")) %>% 
  mutate(annot_treg = annotations_fine)
...
# Create grouped dataframe, calculate the frequencies of clusters
df4 <- seurat_meta_data %>% 
  group_by(MouseID2, seurat_clusters) %>% 
  summarise(n = n()) %>% 
  unique() %>% 
  mutate(freq = n / sum(n)) 
...
ggsave("figs/treg_quant_spl.svg", width = 32, height = 9, units = "cm")
...
ggsave("figs/treg_quant_pLN.svg", width = 32, height = 9, units = "cm")
...
ggsave("figv06/treg_quant_diet_pLN_wilcox.png", width = 32, height = 9, units = "cm")
ggsave("figv06/treg_quant_diet_pLN_wilcox.svg", width = 32, height = 9, units = "cm")
...
df4 %>% 
 ...
 ggsave("figv06/treg_quant_diet_SPL_wilcox.svg", width = 32, height = 9, units = "cm")

ggsave("figv06/treg_quant_diet_SPL_wilcox.png", width = 32, height = 9, units = "cm")
```
...
```{r}
funda_merge$annotations_coarse %>% table
cd4 <- subset(funda_merge, annotations_coarse %in% c("CD4 Naive",
                                                     "CD4 Treg and Tfh",
                                                     "CD4 Egr Myb",
                                                     "CD4 Early act. 41BB CD69"))

cd4$annotations_coarse %>% table

cd4$barcode <- colnames(cd4)
cd4@meta.data <- left_join(cd4@meta.data, dplyr::select(tregs_tfh@meta.data, barcode, annot_treg))

rownames(cd4@meta.data) <- colnames(cd4)
```

```{r}
DimPlot(tregs_tfh, label = T, label.size = 9)
DimPlot(cd4, group.by = "annot_treg")
```

```{r fig.width = 12, fig.height = 4.5}
seurat_meta_data <- cd4@meta.data
seurat_meta_data$MouseID2 <- paste(str_replace_all(string = seurat_meta_data$MouseID, 
                                            pattern = "Mouse_", 
                                            replacement = ""),seurat_meta_data$Tissue)
...
ggsave("figv06/treg_quant_diet_pLN_wilcox_from_cd4.png", width = 32, height = 9, units = "cm")
ggsave("figv06/treg_quant_diet_pLN_wilcox_from_cd4.svg", width = 32, height = 9, units = "cm")
...
df4 %>% 
 ...
 ggsave("figv06/treg_quant_diet_SPL_wilcox_from_cd4.svg", width = 32, height = 9, units = "cm")

ggsave("figv06/treg_quant_diet_SPL_wilcox_from_cd4.png", width = 32, height = 9, units = "cm")
```
...
```
```{r}
FeaturePlot(tgd, features = c("Itga4"), pt.size = 2)
FeaturePlot(tgd, features = c("Klrk1"), pt.size = 2)
FeaturePlot(tgd, features = c("Cd44"), pt.size = 2)



FeaturePlot(tgd, features = c("Cd44", "Sell", "Itga4", "Klrk1", "Trbc2", "Cd4"), pt.size = 1, ncol = 3)
...
```{r}
FeaturePlot(tgd, features = c("Cd44"), pt.size = 12, raster = T,
        raster.dpi = c(1000,1000)) + ggtheme() + theme(axis.line = element_blank(), 
                                       axis.title = element_blank(),  
                                       title = element_text(size = 24),
                                       axis.text = element_blank(), axis.ticks = element_blank())
ggsave("figv06/tgd_Cd44.svg", width = 13, height = 10, units = "cm")
FeaturePlot(tgd, features = c("Sell"), pt.size = 12, raster = T,
        raster.dpi = c(1000,1000)) + ggtheme() + theme(axis.line = element_blank(), 
                                       axis.title = element_blank(),  
                                       title = element_text(size = 24),
                                       axis.text = element_blank(), axis.ticks = element_blank())
ggsave("figv06/tgd_Sell.svg", width = 13, height = 10, units = "cm")
FeaturePlot(tgd, features = c("Cd4"), pt.size = 12, raster = T,
        raster.dpi = c(1000,1000)) + ggtheme() + theme(axis.line = element_blank(), 
                                       axis.title = element_blank(),  
                                       title = element_text(size = 24),
                                       axis.text = element_blank(), axis.ticks = element_blank())
ggsave("figv06/tgd_Cd4.svg", width = 13, height = 10, units = "cm")
FeaturePlot(tgd, features = c("Itga4"), pt.size = 12, raster = T,
        raster.dpi = c(1000,1000)) + ggtheme() + theme(axis.line = element_blank(), 
                                       axis.title = element_blank(),  
                                       title = element_text(size = 24),
                                       axis.text = element_blank(), axis.ticks = element_blank())
ggsave("figv06/tgd_Itga4.svg", width = 13, height = 10, units = "cm")
FeaturePlot(tgd, features = c("Trbc1"), pt.size = 12, raster = T,
        raster.dpi = c(1000,1000)) + ggtheme() + theme(axis.line = element_blank(), 
                                       axis.title = element_blank(),  
                                       title = element_text(size = 24),
                                       axis.text = element_blank(), axis.ticks = element_blank())
ggsave("figv06/tgd_Trbc1.svg", width = 13, height = 10, units = "cm")
FeaturePlot(tgd, features = c("Klrk1"), pt.size = 12, raster = T,
        raster.dpi = c(1000,1000)) + ggtheme() + theme(axis.line = element_blank(), 
                                       axis.title = element_blank(),  
                                       title = element_text(size = 24),
                                       axis.text = element_blank(), axis.ticks = element_blank())
ggsave("figv06/tgd_Klrk1.svg", width = 13, height = 10, units = "cm")
```
...
avgexp = AverageExpression(tgd, features = c("Sell","Cd27","Id3","Trdc","Trgv2","Itga4","Klrd1","Sell",
                                             ...
                                             "Tbx21","Ifng","Il4","Cxcr6","Cxcr3","Klrk1","Fasl","Id2","Cd44","Il2ra","Icos","Il7r","Gata3","Zbtb16","Rorc", "Klrg1",
                                             "Ctla4","Nrp1"),
                           return.seurat = F, group.by = "seurat_clusters", 
                          assay = "RNA")
...
pheatmap(t(avgexp$RNA)[c(1,4,5,2,3),], main = "",
        ...
        color=colorRampPalette(c("dodgerblue", "grey95", "indianred2"))(50),
         border_color = "white",
         fontsize = 12, 
        filename = "figv06/heatmap_tgd_v07.pdf",
         width = 6, height = 2.2,)
```
...
df5 <- df4 %>% 
  dplyr::select(-n) %>% 
  pivot_wider(names_from = seurat_clusters, names_prefix = "cl", values_from = freq, values_fill = 0) %>% 
  mutate(ratio30 = cl3/cl0, ratio40 = cl4/cl0, ratio12 = cl1/cl2) 
  

df5 %>% 
  ggplot(aes(x = Diet, y = ratio30)) +
geom_boxplot(outlier.shape = NA, aes(fill = Diet), alpha = 0.7) + 
  geom_dotplot(binaxis='y', stackdir='center', dotsize=0) + 
  geom_jitter(width = 0.1, height = 0, shape = 21,
              size = 4, aes(fill = Diet)) + # in aes, you can also use shape or fill (for the shapes that allow it)
 facet_wrap(~Tissue, scales = "free", ncol = 62) +
  scale_fill_manual(values = c("dodgerblue","indianred2")) + 
  scale_color_manual(values = c("dodgerblue","indianred2")) + 
  ylab("Frequency") +
  xlab("Condition") +
...
df4 %>% 
  ...
  ggsave("figs/tgd_diet_spleen_1314.svg", width = 32, height = 9, units = "cm")
...
df4 %>% 
  ...
  ggsave("figs/tgd_diet_pLN_1314.svg", width = 27, height = 9, units = "cm")
...
df4 %>% 
  ...
  ggsave("figs/tgd_diet_spleen_ttest.svg", width = 27, height = 9, units = "cm")
...
df4 %>% 
  ...
  ggsave("figs/tgd_diet_pLN_ttest.svg", width = 24, height = 9, units = "cm")
