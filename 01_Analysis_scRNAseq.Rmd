...
avgexp = AverageExpression(funda_merge, features = rev(mrk_de_whole_data$gene),
...
pheatmap(t(avgexp$RNA), main = "",
        ...
        color=colorRampPalette(c("dodgerblue", "grey95", "indianred2"))(50),
         border_color = "white",
         filename = "plots/heatmap_DEgenes_whole_dataset.pdf",
...
for(i in 1:10){
  ...
  annot = levels(factor(funda_merge$annotations_coarse))[i]
  seusubset <- subset(funda_merge, annotations_coarse == annot)
  df_one_sample <- FindAllMarkers(seusubset, only.pos = TRUE, logfc.threshold = 0.1, min.pct = 0.05)
  
  if(nrow(df_one_sample)>0){
  df_one_sample$annotations_coarse <- annot
  
      if(i == 1){
      df_all <- df_one_sample
    } else {
      df_all <- rbind(df_all, df_one_sample)
      }
    }
...
df_all$annotations_coarse %>% table

df_all

df_all_filt <- df_all %>% filter(p_val_adj < 0.2)
...
df_all_filt  %>% 
ggplot(aes(x = factor(gene), y = annotations_coarse)) + 
  geom_point(aes(size = -log(p_val_adj, base = 10), colour = cluster)) + 
...
for(i in 1:2){
  ...
  annot = levels(factor(funda_merge$Tissue))[i]
  seusubset <- subset(funda_merge, Tissue == annot)
  df_one_sample <- FindAllMarkers(seusubset, only.pos = TRUE, logfc.threshold = 0.1, min.pct = 0.05)
  
  if(nrow(df_one_sample)>0){
  df_one_sample$Tissue <- annot
...
options(repr.plot.height = 40, repr.plot.width = 10)
df_all_filt  %>% 
ggplot(aes(x = factor(gene), y = Tissue)) + 
  geom_point(aes(size = -log(p_val_adj, base = 10), colour = cluster)) + 
  theme_bw()  + scale_size_continuous(range=c(1,4)) +
  #scale_colour_gradient2(low = "lightskyblue", mid = "lightsteelblue2", high = "salmon") + 
 theme(axis.text.x = element_text(angle = 90))
...
```{r}
gfd_up_spl <- df_all %>% filter(cluster == "GFD" & Tissue == "Spleen") %>% pull(gene)

gfd_down_spl <- df_all %>% filter(cluster == "Standard" & Tissue == "Spleen") %>% pull(gene)

gfd_up_pln <- df_all %>% filter(cluster == "GFD" & Tissue == "pLN") %>% pull(gene)

gfd_down_pln <- df_all %>% filter(cluster == "Standard" & Tissue == "pLN") %>% pull(gene)
...
ggVennDiagram(genes_up)
ggVennDiagram(genes_down)

venn_up <- VennDiagram::get.venn.partitions(genes_up)
venn_down <- VennDiagram::get.venn.partitions(genes_down)
...
Read data from Schietinger 2022 Nature

```{r}
schietinger <- readRDS("~/PRIMUS/data/48_lab/Project scRNAseq/Analysis of previously published data/028_GSE151652_NOD_Dia/Processed/processed_from_publication/GSE151652_sce_integrated_pLNSamples_filtered.rds")

source("~/PRIMUS/data/48_lab/Project scRNAseq/Scripts Verca/2022-10-17_Funda_NOD/VDJ_utilities.R")
schietinger <- as.Seurat(schietinger)

```

```{r}
tcr_seq_schietinger <- readRDS("~/PRIMUS/data/48_lab/Project scRNAseq/Analysis of previously published data/028_GSE151652_NOD_Dia/Processed/processed_from_publication/GSE151652_tcr_perCell_pairedSamples_newCID_2021-05.rds")

md_schietinger <- schietinger@meta.data 
md_schietinger$cell <- colnames(schietinger)
md_schietinger <- left_join(md_schietinger, tcr_seq_schietinger)

schietinger@meta.data <- md_schietinger
rownames(schietinger@meta.data) <- colnames(schietinger)

schietinger <- NormalizeData(object = schietinger)
  schietinger <- ScaleData(schietinger, verbose = FALSE)
  schietinger <- FindVariableFeatures(schietinger, selection.method = "vst", nfeatures = 1000, 
                                       verbose = FALSE)
  schietinger <- RunPCA(schietinger, npcs = 10, verbose = FALSE)
  schietinger <- RunUMAP(schietinger, reduction = "pca", dims = 1:10)
  schietinger <- FindNeighbors(schietinger, dims = 1:10)
  schietinger <- FindClusters(schietinger, resolution = 0.6)
  DimPlot(schietinger, label = T)
  
```

Process the TCR data from Schietinger to be compatible with ours.

```{r}

md_schietinger2 <- md_schietinger %>%
  separate(
    chains,
    into = c("chain1", "chain2", "chain3", "chain4"),
    sep = ";",
    remove = F) %>%
  separate(
    cdr3_nt,
    into = c("cdr3_nt1", "cdr3_nt2", "cdr3_nt3", "cdr3_nt4"),
    sep = ";",
    remove = F) %>%
  separate(
    cdr3,
    into = c("cdr31", "cdr32", "cdr33", "cdr34"),
    sep = ";",
    remove = F) %>%
  separate(
    v_gene,
    into = c("v_gene1", "v_gene2", "v_gene3", "v_gene4"),
    sep = ";",
    remove = F) %>%
  separate(
    d_gene,
    into = c("d_gene1", "d_gene2", "d_gene3", "d_gene4"),
    sep = ";",
    remove = F) %>%
  separate(
    j_gene,
    into = c("j_gene1", "j_gene2", "j_gene3", "j_gene4"),
    sep = ";",
    remove = F) %>%
  mutate(
    cdr3_A1 = case_when(
      n_chains == 1 & chain1 == "TRA" ~ cdr31,
      n_chains == 2 ~ cdr31,
      n_chains == 3 &
        chain2 == "TRA" ~ cdr31,
      TRUE ~ NA_character_),
    cdr3_A1_nt = case_when(
      n_chains == 1 & chain1 == "TRA" ~ cdr3_nt1,
      n_chains == 2 ~ cdr3_nt1,
      n_chains == 3 &
        chain2 == "TRA" ~ cdr3_nt1,
      TRUE ~ NA_character_),
    cdr3_A2 = case_when(n_chains == 3 &
                          chain2 == "TRA" ~ cdr32,
                        TRUE ~ NA_character_),
    cdr3_A2_nt = case_when(n_chains == 3 &
                             chain2 == "TRA" ~ cdr3_nt2,
                           TRUE ~ NA_character_),
    cdr3_B = case_when(
      n_chains == 1 & chain1 == "TRB" ~ cdr31,
      n_chains == 2 ~ cdr32,
      n_chains == 3 &
        chain2 == "TRA" ~ cdr33,
      TRUE ~ NA_character_),
    cdr3_B_nt = case_when(
      n_chains == 1 & chain1 == "TRB" ~ cdr3_nt1,
      n_chains == 2 ~ cdr3_nt2,
      n_chains == 3 &
        chain2 == "TRA" ~ cdr3_nt3,
      TRUE ~ NA_character_),
    v_gene_A1 = case_when(
      n_chains == 1 & chain1 == "TRA" ~ v_gene1,
      n_chains == 2 ~ v_gene1,
      n_chains == 3 &
        chain2 == "TRA" ~ v_gene1,
      TRUE ~ NA_character_),
    v_gene_A2 = case_when(n_chains == 3 &
                            chain2 == "TRA" ~ v_gene2,
                          TRUE ~ NA_character_),
    v_gene_B = case_when(
      n_chains == 1 & chain1 == "TRB" ~ v_gene1,
      n_chains == 2 ~ v_gene2,
      n_chains == 3 &
        chain2 == "TRA" ~ v_gene3,
      TRUE ~ NA_character_),
    j_gene_A1 = case_when(
      n_chains == 1 & chain1 == "TRA" ~ j_gene1,
      n_chains == 2 ~ j_gene1,
      n_chains == 3 &
        chain2 == "TRA" ~ j_gene1,
      TRUE ~ NA_character_),
    j_gene_A2 = case_when(n_chains == 3 &
                            chain2 == "TRA" ~ j_gene2,
                          TRUE ~ NA_character_),
    j_gene_B = case_when(
      n_chains == 1 & chain1 == "TRB" ~ j_gene1,
      n_chains == 2 ~ j_gene2,
      n_chains == 3 &
        chain2 == "TRA" ~ j_gene3,
      TRUE ~ NA_character_),
    d_gene_A1 = case_when(
      n_chains == 1 & chain1 == "TRA" ~ d_gene1,
      n_chains == 2 ~ d_gene1,
      n_chains == 3 &
        chain2 == "TRA" ~ d_gene1,
      TRUE ~ NA_character_),
    d_gene_A2 = case_when(n_chains == 3 &
                            chain2 == "TRA" ~ d_gene2,
                          TRUE ~ NA_character_),
    d_gene_B = case_when(
      n_chains == 1 & chain1 == "TRB" ~ d_gene1,
      n_chains == 2 ~ d_gene2,
      n_chains == 3 &
        chain2 == "TRA" ~ d_gene3,
      TRUE ~ NA_character_))

schietinger@meta.data <- md_schietinger2
rownames(schietinger@meta.data) <- colnames(schietinger)
```
...
```{r}
 collier <- readRDS("~/PRIMUS/data/48_lab/Project scRNAseq/Analysis of previously published data/075_Collier_NOD/Processed/GSE228233_all_cells_integrated.rds")

DimPlot(collier, label = T)

collier$schietinger_NRPv7_tetramer_specificity
```

```{r}
colnames(collier@meta.data)
```

```{r}
md_collier <- collier@meta.data

md_collier2 <- md_collier %>%
   mutate(
    cdr3_A1 = TRA.cdr3,
    cdr3_A2_nt = TRA.cdr3_nt,
    cdr3_B = TRB.cdr3,
    cdr3_B_nt = TRB.cdr3_nt,
    v_gene_A1 = TRA.v_gene,
   v_gene_B = TRB.v_gene,
    j_gene_A1 = TRA.j_gene,
    j_gene_B = TRB.j_gene,
    d_gene_A1 = TRA.d_gene,
    d_gene_B = TRB.d_gene)

collier@meta.data <- md_collier2
rownames(collier@meta.data) <- colnames(collier)
```

```{r}
DimPlot(collier, group.by = "d_gene_B")

collier$annotation <- Idents(collier)
Idents(collier) %>% table
```


```{r}
collier$Study <- "Collier"
schietinger$Study <- "Schietinger"
funda_merge$Study <- "Lab48"


schietinger@assays$RNA <- schietinger@assays$originalexp

all.nod <- list(funda_merge, collier, schietinger)

all.nod

all.nod <- merge(funda_merge, c(collier, schietinger))
all.nod <- SplitObject(all.nod, split.by = "Study")


merged.list <- lapply(X = all.nod, FUN = function(x) {
    DefaultAssay(x)  <- "RNA"
...
nod.anchors <- FindIntegrationAnchors(object.list = merged.list, anchor.features = features, reduction = "rpca")

# this command creates an 'integrated' data assay
nod.combined <- IntegrateData(anchorset = nod.anchors)

# Run the standard workflow for visualization and clustering
nod.combined <- ScaleData(nod.combined, verbose = FALSE)
nod.combined <- RunPCA(nod.combined, npcs = 10, verbose = FALSE)
nod.combined <- RunUMAP(nod.combined, reduction = "pca", dims = 1:10)

DimPlot(nod.combined)

options(repr.plot.width = 7, repr.plot.height = 6)

DimPlot(nod.combined, group.by = "Study")
DimPlot(nod.combined, group.by = "annotations_fine", label = T, split.by = "Study")
DimPlot(nod.combined, group.by = "annotations", label = T)
...
collier <- subset(collier, annotation != "Acinar_contaminated")
DimPlot(collier)

collier$Study <- "Collier"
schietinger$Study <- "Schietinger"
funda_merge$Study <- "Lab48"
collier$annotation %>% table
...
nod.combined <- Run.STACAS(merged.list, dims = 1:12)
nod.combined <- RunUMAP(nod.combined, dims = 1:12) 

DimPlot(nod.combined, group.by = "Study")
DimPlot(nod.combined, group.by = "annotations_fine", label = T, split.by = "Study")
DimPlot(nod.combined, group.by = "annotation", label = T)
...
convT <- subset(funda_merge, Immgen_annot_single_main == "T cells" & seurat_clusters %in% c(0:8,11:18))
 ...
 convT <- NormalizeData(object = convT)
  ...
  md <-
    md %>% mutate(
      cd4_or_cd8 = if_else(
        cd4_expression == F & cd8a_expression == T & cd8b_expression == T,
        "CD8",
        if_else(
          cd4_expression == T &
            cd8a_expression == F & cd8b_expression == F,
          "CD4",
          "Unclear")))
    ...
    md %>% mutate(
      cd4_or_cd8_2 = if_else(
        ...
        if_else(
          ...
          "CD4",
          if_else(
            seurat_clusters %in% c(0, 1, 3, 4, 6, 7, 8, 9, 12, 13, 14, 15),
            "CD4",
            "CD8"))))
...
cd8$source <- "NOD CD8 Exp11"
nod_atlas$source <- "NOD CD8 Exp05"
nod_atlas$Tissue <- "Spleen"
schietinger$source <- "Schietinger 2022"

schietinger[["RNA"]] <- schietinger[["originalexp"]]
DefaultAssay(schietinger) <- "RNA"

nod_merge <- merge(cd8, c(nod_atlas, schietinger))
    nod_integrated_cd8 <- SplitObject(nod_merge, split.by = "source")
    nod_integrated_cd8 <- Run.STACAS(nod_integrated_cd8, dims = 1:10, anchor.features = 1200)
    set.seed(1024)
    nod_integrated_cd8 <- RunUMAP(nod_integrated_cd8, dims = 1:10)
     nod_integrated_cd8 <- FindNeighbors(nod_integrated_cd8, dims = 1:10)
  nod_integrated_cd8 <- FindClusters(nod_integrated_cd8, resolution = 0.8)

    
# saveRDS(nod_integrated_cd8, "data/nod_integrated_cd8.rds")
      
DimPlot(nod_integrated_cd8, group.by = c("source"), split.by = "source")
DimPlot(nod_integrated_cd8, group.by = c("population"), split.by = "source")
DimPlot(nod_integrated_cd8, group.by = c("Tissue"), split.by = "source")
```

Make compatible metadata

```{r}
md <- nod_integrated_cd8@meta.data

md <- md %>% mutate(MouseID_2 = if_else(source == "NOD CD8 Exp05", mouse_number,
                              if_else(source == "NOD CD8 Exp11", MouseID,
                                              Mouse)),
                    diabetes_2 = if_else(source == "NOD CD8 Exp05", if_else(dia == "no","healthy","dia"),
                              if_else(source == "NOD CD8 Exp11", Diet,
                                              Diabetic.status)))
```

```{r}
df4 <- md %>% 
  mutate(MouseID_tissue = paste(MouseID_2, Tissue)) %>% 
  group_by(MouseID_tissue, seurat_clusters) %>% 
 summarise(n = n()) %>%
  mutate(freq = n / sum(n)) 
...
md_to_join <- md %>% 
  mutate(MouseID_tissue = paste(MouseID_2, Tissue)) %>% 
  dplyr::select(source, MouseID_tissue, diabetes_2, age_weeks, Tissue) %>% 
  unique
  
df4 <- df4 %>% left_join(md_to_join)
```


```{r fig.width=19, fig.height=3}
p2 <-    df4 %>%
  unique %>% 
   filter(source == "NOD CD8 Exp05") %>% 
   mutate(age = if_else(age_weeks < 10, "6-8wk", "16-27wk")) %>% 
ggplot(aes(x = factor(diabetes_2, levels = c("healthy","dia")), y = freq*100)) + 
 geom_boxplot(outlier.shape = NA) +
 geom_dotplot(binaxis='y', stackdir='center', dotsize=0) + geom_jitter(position=position_jitter(0.2), size = 2, aes(color = factor(age))) + scale_color_manual(values= c("firebrick","lightskyblue3")) + 
   theme_minimal() + 
      facet_wrap(~seurat_clusters, scales = "free", ncol = 12) +
      theme(axis.text.x = element_text(angle = 90))
 
 
  p3 <-    df4 %>%
   filter(source == "NOD CD8 Exp11" & Tissue == "pLN") %>% 
ggplot(aes(x = factor(diabetes_2), y = freq*100)) + 
 geom_boxplot(outlier.shape = NA) +
 geom_dotplot(binaxis='y', stackdir='center', dotsize=0) + geom_jitter(position=position_jitter(0.2), size = 2, aes(color = factor(diabetes_2))) + scale_color_manual(values= c("firebrick","lightskyblue3")) + 
   theme_minimal() + 
     facet_wrap(~seurat_clusters, scales = "free", ncol = 12) +
       theme(axis.text.x = element_text(angle = 90))
  
   p3b <-    df4 %>%
   filter(source == "NOD CD8 Exp11" & Tissue == "Spleen") %>% 
ggplot(aes(x = factor(diabetes_2), y = freq*100)) + 
 geom_boxplot(outlier.shape = NA) +
 geom_dotplot(binaxis='y', stackdir='center', dotsize=0) + geom_jitter(position=position_jitter(0.2), size = 2, aes(color = factor(diabetes_2))) + scale_color_manual(values= c("firebrick","lightskyblue3")) + 
   theme_minimal() + 
   ...
   p4 <-    df4 %>%
   filter(source == "Schietinger 2022") %>% 
ggplot(aes(x = factor(diabetes_2), y = freq*100)) + 
 geom_boxplot(outlier.shape = NA) +
 geom_dotplot(binaxis='y', stackdir='center', dotsize=0) + geom_jitter(position=position_jitter(0.2), size = 2, aes(color = factor(Tissue))) + scale_color_manual(values= c("firebrick","lightskyblue3")) + 
   theme_minimal() + 
      facet_wrap(~seurat_clusters, scales = "free", ncol = 12) +
      theme(axis.text.x = element_text(angle = 90))
...
## TCR repertoires in merged dataset

## Clone abundance
```{r}
metadata_6 <- nod_integrated_cd8@meta.data
rownames(metadata_6) <- colnames(nod_integrated_cd8)
...
```{r}
top_88_clones <- pull(clone_table, `factor(clone_aa)`)[2:88]

metadata_7 <- metadata_6 %>% filter(clone_aa %in% top_88_clones) %>% group_by(clone_aa, source) %>% tally %>% pivot_wider(names_from = "source", values_from = n, values_fill = 0) 
metadata_7

metadata_7 %>% mutate(h1 = if_else(`NOD CD8 Exp05`>0,1,0), 
                      h2 = if_else(`NOD CD8 Exp11`>0,1,0), 
                      h3 = if_else(`schietinger 2022`>0,1,0)) %>% 
  mutate(sum = sum(h1,h2,h3)) %>% dplyr::select(1,2,3,4,8) %>% arrange(desc(sum))
```

Two important receptors:
CDR3b CASSDSQNTLYF CDR3a CAMRDSGGSNAKLTF
CDR3b CASSRDSSYEQYF CDR3a CAMRLSGGSNAKLTF


```{r}
DimPlot(funda_merge, 
        cells.highlight = colnames(funda_merge)[which(funda_merge$clone_aa == "CDR3b CASSDSQNTLYF CDR3a CAMRDSGGSNAKLTF")],
        sizes.highlight = 2) + NoLegend() + ggtitle(str_replace(string = "CDR3b CASSDSQNTLYF CDR3a CAMRDSGGSNAKLTF", pattern = "CDR3a", replacement = "\nCDR3a"))

DimPlot(nod_integrated_cd8, 
        cells.highlight = colnames(nod_integrated_cd8)[which(nod_integrated_cd8$clone_aa == "CDR3b CASSDSQNTLYF CDR3a CAMRDSGGSNAKLTF")],
        sizes.highlight = 2) + NoLegend() + ggtitle(str_replace(string = "CDR3b CASSDSQNTLYF CDR3a CAMRDSGGSNAKLTF", pattern = "CDR3a", replacement = "\nCDR3a"))
```
...
```{r}
DimPlot(funda_merge, 
        cells.highlight = colnames(funda_merge)[which(grepl(funda_merge$cdr3_A1, pattern = "CAMRDSGGSNAKLTF"))],
        sizes.highlight = 2) + NoLegend() + ggtitle(str_replace(string = "TCR including CAMRDSGGSNAKLTF", pattern = "CDR3a", replacement = "\nCDR3a"))

clone_table %>% filter(grepl(`factor(clone_aa)`, pattern = "CAMR.SGGSNAKLTF"))

DimPlot(funda_merge, 
        cells.highlight = colnames(funda_merge)[which(grepl(funda_merge$cdr3_A1, pattern = "CAMR.SGGSNAKLTF"))],
        sizes.highlight = 2) + NoLegend() + ggtitle(str_replace(string = "TCR including CAMR.SGGSNAKLTF", pattern = "CDR3a", replacement = "\nCDR3a"))
```
...
metadata_6 <- as.data.frame(metadata_6 %>% mutate(clone_abundance = as.numeric(ifelse(clone_abundance==4991,NA_integer_,clone_abundance))) %>%
  mutate(log_clone_abundance = log(clone_abundance, base = 2)))
rownames(metadata_6) <- colnames(nod_integrated_cd8)

nod_integrated_cd8@meta.data <- metadata_6
rownames(nod_integrated_cd8@meta.data) <- colnames(nod_integrated_cd8)

nod_integrated_cd8 <- AddMetaData(nod_integrated_cd8, as.numeric(metadata_6$clone_abundance), "clone_abundance")
rownames(nod_integrated_cd8@meta.data) <- colnames(nod_integrated_cd8)

FeaturePlot(nod_integrated_cd8, reduction = "umap", features = "log_clone_abundance")

FeaturePlot(subset(nod_integrated_cd8, clone_abundance >=1 & clone_abundance < 4991), reduction = "umap", features = "log_clone_abundance", cols = c("lightblue","firebrick")) + theme_classic() + theme(plot.title = element_text(hjust = 0.5)) + ggtitle("Log2 clone abundance")
#ggsave("fig/Log2CloneAbundance.png", width = 13, height = 8, units = "cm", dpi = 120)
#ggsave("fig/Log2CloneAbundance.svg", width = 13, height = 8, units = "cm", dpi = 120)

# Clone abundance bar graph
metadata_6 <- metadata_6 %>%
  mutate( clone_abundance_group = if_else(clone_abundance>10, "10+",
                                          if_else(clone_abundance==4, "6-9",
                                             if_else(clone_abundance==3, "3-5",
                                             if_else(clone_abundance==2, "2",
                                             if_else(clone_abundance==1, "1","1"))))))

metadata_6 %>% 
   ggplot(aes(x = annotations_fine)) + geom_bar(aes(fill = factor(clone_abundance_group)), position = "fill") + coord_flip() +
